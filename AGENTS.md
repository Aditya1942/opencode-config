...
plugins/my-skills.js     # Bootstrap plugin — injects skill framework via system prompt
skills/
  my-skills/             # Consolidated workflow and domain skills (TDD, debugging, planning, etc.)
...
| Command | Purpose |
|---------|---------|
| `/which-skill` | Auto-detect best skill(s) for current task (invokes skill-chooser) |
...
| `/readme-first` | README-first workflow: read nearest README before code; keep README in sync after changes |
| `/init-readme` | Initialize or refresh README.md for a package/class/module (readme skill + CLI) |
| `/remember-this` | Save compact memory (decisions, gotchas, patterns) for current module via memory MCP |
| `/recall` | Retrieve stored memory for a module or topic from the knowledge graph |
...
| `ultron` | Claude Sonnet | subagent | Planning sub-agent (planner merged): requirements, architecture review, phases, skill-chooser + worker-selection per step. Outputs structured plan (steps, skills, worker, verification, testing, risks). Do not execute. |
...
| `code-reviewer` | Claude Sonnet | subagent | Code review specialist: quality, security, maintainability. Use after writing or modifying code; required before claiming work done. |
...
| `subagent-driven-development` | **Orchestrator always.** All work via subagents or worker CLI via shell; never perform task directly. Independent tasks in current session. |
