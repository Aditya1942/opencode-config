...
3. **Assigns skills directly** for the overall task and per step
...
- You need a **plan with per-step skill recommendations** (direct selection).
...
|-----------|--------|------------------------|
| **Bite-sized TDD steps** | plan-writing, TDD best practices | Each task broken into: write failing test → confirm fail → implement minimal code → confirm pass → commit. Each step 2–5 minutes. |
...
- **skill selection:** Assigns 1–3 skills per task.
...
**What to expect:** Ultron should (1) optionally use tools to explore/summarize the repo, (2) assign skills per task and per step (e.g. api-design, testing, readme), (3) write a plan file to `docs/plans/` with bite-sized TDD steps per task, (4) present the plan and offer Subagent-Driven vs Manual execution.
