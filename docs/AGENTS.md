...
**Scope:** The 12 agents defined in opencode.json.
...
| **orchestrator** | user-selected | primary | **PURE dispatcher:** never performs tasks directly. Routes by task type: explore → @explore; big/multi-step → @sequencer then @executor; small → delegate or perform via subagent. |
...
| **cursor-explorer** | Claude Sonnet | subagent | Read-only exploration, summarization, brainstorming, architecture analysis via cursor_agent (plan/ask mode). Cursor-native alternative to @explore. |
| **cursor-general** | Claude Sonnet | subagent | General-purpose execution workhorse via cursor_agent (agent mode): implementation, coding tasks, plan execution. Cursor-native alternative to @executor. |
| **cursor-reviewer** | Claude Sonnet | subagent | Code review specialist via cursor_agent (ask/plan mode): quality, security, maintainability. Read-only. Use after code changes before marking done. |
...
- **Before completion:** Validator and @cursor-reviewer or @code-reviewer must run (via executor or directly).
- **Cursor subagents:** For Cursor-native work, route exploration → @cursor-explorer, execution → @cursor-general, review → @cursor-reviewer instead of (or in addition to) @explore, @executor, @code-reviewer.
